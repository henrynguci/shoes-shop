...
class PromotionOrderSeeder {
  static async run() {
    ...
    try {
      ...
      const promotionIds = result.recordset.map(
        (record) => record.Promotion_ID
      );

      const selectedPromotionIds = promotionIds.slice(
        0,
        Math.floor(promotionIds.length / 2)
      );

      const promotionOrders =
        PromotionOrderFactory.generate(selectedPromotionIds);
      ...
      for (const order of promotionOrders) {
        await pool
          ...
          .input('Max_price', order.Max_price).query(`
